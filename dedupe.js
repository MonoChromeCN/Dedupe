...
function operator(pro) {
    // ===================================
  // [START] 新增：基于配置信息进行节点去重
  // ===================================
  const dedupeMap = new Map();

  pro.forEach((proxy) => {
    // 1. 复制所有节点属性到新对象，作为生成 Key 的基础
    const keyObject = {};
    ...
    // 2. 移除不参与比对的属性 (Name, UUID, Password)
    // 只有这些属性不同，其他完全一致的节点会被视为重复
    ...
    // 3. 将剩余属性对象转换为一个稳定的字符串 Key
    // JSON.stringify 确保了除了忽略的属性外，配置完全一致的节点会得到相同的 Key
    const key = JSON.stringify(keyObject);

    // 4. 使用 Map 记录唯一节点
    // 只有当 Map 中不存在该 Key 时，才将该原始节点保留下来（Map只会保留第一个遇到的节点）
  ...
  // 5. 用去重后的节点列表替换原始列表
  // 只有唯一配置的节点才会进入后续的重命名流程
  pro = Array.from(dedupeMap.values());
  // ===================================
  // [END] 新增：基于配置信息进行节点去重
  // ===================================
  const Allmap = {};
